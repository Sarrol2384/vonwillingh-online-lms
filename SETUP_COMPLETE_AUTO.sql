-- =====================================================
-- COMPLETE SETUP - ONE SCRIPT DOES EVERYTHING
-- =====================================================
-- Creates tables AND configures Module 1 automatically
-- No validation, no type checking, just creates everything
-- =====================================================
...
-- Step 2: Add columns to modules table
DO $$
BEGIN
...
-- Step 3: Configure Module 1 automatically
DO $$
DECLARE
    v_course_id INTEGER;
...
BEGIN
    ...
    SELECT 
        c.id,
        (SELECT m.id FROM modules m WHERE m.course_id = c.id AND m.title ILIKE '%Module 1%' ORDER BY m.id LIMIT 1),
        (SELECT m.id FROM modules m WHERE m.course_id = c.id AND m.title ILIKE '%Module 2%' ORDER BY m.id LIMIT 1)
...
-- =====================================================
-- DONE! Module 1 is now configured for quiz progression
-- =====================================================
